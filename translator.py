...
def translator_v2ckt(src, dst):
    ...
    for i in range(len(f_content)):
        # create a file only contain input infomation
        if f_content[i].find("input") == 0:
            f.write(f_content[i])
        # create a file only contain output infomation
        elif f_content[i].find("output") == 0:
            f_output.write(f_content[i])
        # create a file only contain wire infomation
    ...
    ##deal with inner gate output and its branch
    for i in range(len(wire_arr)):
        # print(gate_input_num.get(wire_arr[i]))
